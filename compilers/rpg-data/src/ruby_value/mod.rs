...
use rpg_types::{RpgFields, RpgHashKey, RpgValue};
use std::collections::HashMap;

/// 将 alox_48::Value 转换为 RpgValue
pub fn value_to_rpg(value: &Value) -> RpgValue {
    match value {
        ...
        Value::Array(arr) => RpgValue::Array(arr.iter().map(value_to_rpg).collect()),
        Value::Hash(hash) => {
            let mut map = HashMap::new();
            for (k, v) in hash {
                map.insert(RpgHashKey(value_to_rpg(k)), value_to_rpg(v));
            }
            RpgValue::Hash(map)
        }
        ...
        Value::Instance(i) => {
            RpgValue::Instance { value: Box::new(value_to_rpg(i.value.as_ref())), fields: convert_fields(&i.fields) }
        }
        ...
        Value::Extended { module, value } => {
            RpgValue::Extended { module: module.to_string(), value: Box::new(value_to_rpg(value.as_ref())) }
        }
        Value::UserClass { class, value } => {
            RpgValue::UserClass { class: class.to_string(), value: Box::new(value_to_rpg(value.as_ref())) }
        }
        Value::UserMarshal { class, value } => {
            RpgValue::UserMarshal { class: class.to_string(), value: Box::new(value_to_rpg(value.as_ref())) }
        }
        Value::Data { class, value } => {
            RpgValue::Data { class: class.to_string(), value: Box::new(value_to_rpg(value.as_ref())) }
        }
    }
}

/// 将 RpgValue 转换为 alox_48::Value
pub fn rpg_to_value(value: &RpgValue) -> Value {
    match value {
        ...
        RpgValue::Array(arr) => {
            let values: RbArray = arr.iter().map(rpg_to_value).collect();
            Value::Array(values)
        }
        RpgValue::Hash(map) => {
            let mut hash = RbHash::new();
            for (k, v) in map {
                hash.insert(rpg_to_value(&k.0), rpg_to_value(v));
            }
            Value::Hash(hash)
        }
        ...
        RpgValue::Instance { value, fields } => {
            Value::Instance(Instance { value: Box::new(rpg_to_value(value.as_ref())), fields: convert_to_rb_fields(fields) })
        }
        ...
        RpgValue::Extended { module, value } => {
            Value::Extended { module: Symbol::from(module.as_str()), value: Box::new(rpg_to_value(value.as_ref())) }
        }
        RpgValue::UserClass { class, value } => {
            Value::UserClass { class: Symbol::from(class.as_str()), value: Box::new(rpg_to_value(value.as_ref())) }
        }
        RpgValue::UserMarshal { class, value } => {
            Value::UserMarshal { class: Symbol::from(class.as_str()), value: Box::new(rpg_to_value(value.as_ref())) }
        }
        RpgValue::Data { class, value } => {
            Value::Data { class: Symbol::from(class.as_str()), value: Box::new(rpg_to_value(value.as_ref())) }
        }
    }
}
...
fn convert_fields(fields: &RbFields) -> RpgFields {
    let mut map = HashMap::new();
    for (k, v) in fields {
        map.insert(k.to_string(), value_to_rpg(v));
    }
    map
}
...
fn convert_to_rb_fields(fields: &RpgFields) -> RbFields {
    ...
    for (k, v) in fields {
        rb_fields.insert(Symbol::from(k.as_str()), rpg_to_value(v));
    }
    rb_fields
}
